...
export const RuleBuilder = ({ onRulesChange }: Props) => {
  ...
  const add = (r: Rule[]) => {
    ...
    toast.success("Rules added", {
      description: `${r.length} rule(s) merged into your set.`,
    });
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Create co-run, load limits, phase windows and more
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            <Input
              placeholder="TaskIDs comma separated (e.g. T1,T2)"
              id="corun"
            />
            <Button
              onClick={() => {
                const el = document.getElementById("corun") as HTMLInputElement;
                const tasks = (el.value || "")
                  .split(",")
                  .map((s) => s.trim())
                  .filter(Boolean);
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            ...
            <Button
              onClick={() => {
                const g = (
                  document.getElementById("ll-group") as HTMLInputElement
                ).value.trim();
                const m = Number(
                ...
                if (g && m > 0)
                  add([
                    {
                      type: "loadLimit",
                      target: { kind: "WorkerGroup", value: g },
                      maxSlotsPerPhase: m,
                    },
                  ]);
                (
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            ...
            <Input
              placeholder="allowed phases (e.g. 1-3 or 1,3,5)"
              id="pw-phases"
            />
            <Button
              onClick={() => {
                const id = (
                  document.getElementById("pw-id") as HTMLInputElement
                ).value.trim();
                const val = (
                  document.getElementById("pw-phases") as HTMLInputElement
                ).value.trim();
                const phases = val.includes("-")
                  ? (() => {
                ...
                if (id && phases.length)
                  add([
                    { type: "phaseWindow", taskId: id, allowedPhases: phases },
                  ]);
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            ...
            <Button
              onClick={() => {
                const g = (
                  document.getElementById("sr-group") as HTMLInputElement
                ).value.trim();
                const m = Number(
